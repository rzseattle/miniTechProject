...
learning_rate_reduction_factor = 0.4  # the factor used when reducing the learning rate -> learning_rate *= learning_rate_reduction_factor
...
#base_dir = os.path.join('..', '..' ,'Fruit-Images-Dataset')  # relative path to the Fruit-Images-Dataset folder
...
def train_and_evaluate_model(model, name="", epochs=25, batch_size=50, verbose=1, useCkpt=False):
    print(model.summary())
    ...
    save_model = ModelCheckpoint(filepath=model_out_dir + "/model.keras", monitor='loss', verbose=verbose,
    ...
    history = model.fit(trainGen,
                        ...
                        validation_steps=(testGen.n // batch_size) + 1,
                        verbose=verbose,
   ...
   #plot_confusion_matrix(y_true, y_pred.argmax(axis=-1), labels, out_path=model_out_dir)
    class_report = classification_report(y_true, y_pred.argmax(axis=-1), target_names=labels, zero_division=0)
